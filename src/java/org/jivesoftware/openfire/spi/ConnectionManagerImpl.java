...
public class ConnectionManagerImpl extends BasicModule implements ConnectionManager, CertificateEventListener {
    ...
    private synchronized void startListeners() {
        ...
        startUDPClientListeners(localIPAddress);
    ...
    private void createUDPClientListeners() {
        // Start clients plain socket unless it's been disabled.
        if (isClientListenerEnabled()) {
            // Create SocketAcceptor with correct number of processors
            datagramAcceptor = buildDatagramAcceptor();
            // Customize Executor that will be used by processors to process incoming stanzas
            ExecutorThreadModel threadModel = ExecutorThreadModel.getInstance("client");
            int eventThreads = JiveGlobals.getIntProperty("xmpp.client.processing.threads", 16);
            ThreadPoolExecutor eventExecutor = (ThreadPoolExecutor)threadModel.getExecutor();
            ...
            datagramAcceptor.getDefaultConfig().setThreadModel(threadModel);
            // Add the XMPP codec filter
            datagramAcceptor.getFilterChain().addFirst("xmpp", new ProtocolCodecFilter(new XMPPCodecFactory()));
            // Kill sessions whose outgoing queues keep growing and fail to send traffic
            datagramAcceptor.getFilterChain().addAfter("xmpp", "outCap", new StalledSessionsFilter());
        }
    }
    ...
    private void startUDPClientListeners(String localIPAddress) {
        // Start clients plain socket unless it's been disabled.
        if (isClientListenerEnabled()) {
            int port = getUDPClientListenerPort();
            try {
                // Listen on a specific network interface if it has been set.
                String interfaceName = JiveGlobals.getXMLProperty("network.interface");
                InetAddress bindInterface = null;
    ...
    private void stopUDPClientListeners() {
    ...
    public int getUDPClientListenerPort() {
    ...
    private DatagramAcceptor buildDatagramAcceptor() {
        ...
        Executor ioExecutor = new ThreadPoolExecutor(
            ioThreads + 1, ioThreads + 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>() );
        datagramAcceptor = new DatagramAcceptor(ioExecutor);
        // Set that it will be possible to bind a socket if there is a connection in the timeout state
        DatagramAcceptorConfig datagramAcceptorConfig = datagramAcceptor.getDefaultConfig();
        DatagramSessionConfig datagramSessionConfig = datagramAcceptorConfig.getSessionConfig();
       ...
       // socketAcceptorConfig.setBacklog(JiveGlobals.getIntProperty("xmpp.socket.backlog", 50));
        ...
        int receiveBuffer = JiveGlobals.getIntProperty("xmpp.socket.buffer.receive", -1);
        if (receiveBuffer > 0 ) {
        	datagramSessionConfig.setReceiveBufferSize(receiveBuffer);
        }
        int sendBuffer = JiveGlobals.getIntProperty("xmpp.socket.buffer.send", -1);
        if (sendBuffer > 0 ) {
        	datagramSessionConfig.setSendBufferSize(sendBuffer);
        }
